...
def main():
    ...
    M = .01
    dt = 1
    u = np.array([.01,.01]).reshape(2,1)
    P0 = np.diag([50,50,10,10])
    Rv = np.zeros((2,2))
    Rv[[0,1],[0,1]] = 1e-5
    # print(Rv)
    Rw = np.zeros((2,2))
        ...
        while not shutdown:
            for frame in container.decode(video=0):
                ...
                if frame_count %4 == 0:
                    ...
                    #if can only see nose we need to either move backwards or down
                    #want to see shoulders and hips
                    ...
                    if keypoint_scores[0,0] > .04:
                        ...
                        if time.perf_counter() - PREV_PREDICT_TIME > MAX_PREDICT_TIME:
                            ...
                            keypoint_coords[0,0,1] = nosex
                            keypoint_coords[0,0,0] = nosey
                            overlay_image = cv2.line(overlay_image, (centerx,centery - 10), (nosex, nosey), (255, 0, 0), 2)
                            errorx = nosex - centerx
                            errory = nosey - centery - 10
                            if abs(errorx) > 20:
                                ctrl_out_cc = pid_cc(errorx)
                    ...
                    meanHeight = ((rightHipy - rightSholy) + (leftHipy - leftSholy))/2
                    desiredHeight = 100 #technically arbitrary
                    ctrl_out_fb = 0
                    ...
                    if keypoint_scores[0,5] > .04 and keypoint_scores[0,6] > .04 and keypoint_scores[0,11] > .04 and keypoint_scores[0,12] > .04:
                        ...
                        #reset pid
                        #if can see ears use width as reference
                        #if can see shoulders use width as reference
